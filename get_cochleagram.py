...
import scipy
import numpy as np
#many of the functions in this file are adapted from the model_metamers_pytorch repository
...
def get_cochleagram(wav,config=None,cpu=True):
    #wav: the waveform of the sound, a numpy 1D array
    #config: the configuration of the cochleagram, a dictionary
    #cpu: whether to use cpu to compute the cochleagram, a boolean value

    #return the cochleagram, which is a 2D numpy array (frequency,time)
    ...
    if config is None:
        ...
        cochleagram_1 = config

    layer = AudioCompose([
        AudioToTensor(),
        CombineWithRandomDBSNR(low_snr=-10, high_snr=10),
        RMSNormalizeForegroundAndBackground(rms_level=0.1),
        UnsqueezeAudio(dim=0),
        AudioToAudioRepresentation(**cochleagram_1)
        ])
    cochleagram = layer(wav, None)
    cochleagram = cochleagram[0].squeeze(0).detach().cpu().numpy()
...
class AudioToCochleagram(torch.nn.Module):
    ...
    def forward(self, foreground_wav, background_wav):
        ...
        """
        del background_wav
        self.signal_size = foreground_wav.shape[-1]
        self.filters = self.make_coch_filters(self.signal_size,
                                              self.sr, 
                                              **self.coch_filter_kwargs)
        self.Cochleagram = chcochleagram.cochleagram.Cochleagram(self.filters, 
                                                                ...
                                                                self.downsampling_op,
                                                                compression=None)
        self.Cochleagram = self.Cochleagram.to(foreground_wav.device)
        if foreground_wav is not None:
...
if __name__ == '__main__':
    wav = '/scratch/snormanh_lab/shared/sounds/naturalsounds165/wav/stim5_alarm_clock.wav'
    y, sr = sf.read(wav)
    y = scipy.signal.resample(y, np.int32(np.round(20e3/sr * len(y))))
    cpu=True
    pred = get_cochleagram(np.float32(y))
